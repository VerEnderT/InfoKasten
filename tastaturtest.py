...
# >>>>> Hauptprogramm  <<<<<<<
    def __init__(self):
        super().__init__()
        #    self.image = ImageTk.PhotoImage(Image.open("insidepost_coding.jpg"))

        # region variablen
        # Daten einlesen Variablen
        self.lastevent = ""
        self.toolname = "Tastatur Ubuntu/Linux"
        ...
        self.color4 = "#ffffff"
        self.color3 = "yellow"
        self.keycode1 = [9, 67, 68, 69, 70, 71, 72, 73, 74, 75, 76, 95, 96, 78, 127]
        self.keyLine1 = ["Esc", "F1", "F2", "F3", "F4", "F5", "F6", "F7", "F8", "F9", "F10", "F11", "F12",
                         "Rollen", "Pause"]
        ...
        for name in self.keyLine1:
            if name == "F12" or name == "Esc" or name == "F4" or name == "F8":
                ...
                self.labelKey = tk.Label(master=self.frame_a, text=name, fg=self.color2, bg=self.color1, width=5)
                self.labelKey.pack(padx=(53, 4), side="left")
            else:
                self.labelKey = tk.Label(master=self.frame_a, text=name, fg=self.color2, bg=self.color1, width=5)
        ...
        self.labelEnter.place(x=611, y=98)
        ...
        #self.bind("<Tab-Release>", self.key_tab_release)
